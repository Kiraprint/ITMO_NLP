...
class BertClassifier:
    ...
    def __init__(self, model_path, tokenizer_path, n_classes=2, epochs=1, model_save_path='./bert.pt'):
        ...
        # reset classifier layer for needed number of classes
    ...
    def fit(self): # train step
        ...
        # calculate eval metric (accuracy)
        train_acc = correct_predictions.double() / len(self.train_set)
        train_loss = np.mean(losses)
        return train_acc, train_loss
    
    def eval(self): # eval step 
        self.model = self.model.eval()
        losses = []
        correct_predictions = 0

        # don't need to compute gradients, use fastest inference mode (> torch.no_grad)
    ...
    def train(self, early_stop:int): # train loop
        best_accuracy = 0
        stop_count = 0
        ...
        self.model = torch.load(self.model_save_path) # use best model
